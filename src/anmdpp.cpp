...
int hist_mdpp_init()
{
	...
	test_2d_hist = H2_BOOK("test_2d_handle", "test_2d_chan", TEST_2d_BINS, 0, TEST_2d_BINS);

	// Define a distribution (e.g., for integers between 1 and 19)
    std::uniform_int_distribution<int> dist2d(0, 19);
	
	for(int i=0; i<1000; i++) {
		int rand_x = dist2d(mt); // Generate the random number for x axis
		int rand_y = dist2d(mt); // Generate the random number for y axis
		test_2d_hist->Fill(test_2d_hist, rand_x, rand_y, test_2d_hist->rowl, 1);
    }
